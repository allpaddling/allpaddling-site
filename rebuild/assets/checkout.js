...
(function () {
  ...
  function openAnonModal (planType, planKey, originalBtn) {
    ...
    // Pre-fill from ?email=... URL param if present. Used by the urgent
    // migration emails that link Custom Plan customers straight to
    // /custom-plan.html?email=their.email — saves them from retyping and
    // also keeps continuity (they sign up with the same address we have
    // on file in migration_customers, so the system recognises them).
    // Validates lightly; if the param is junk we just leave the field
    // empty and the customer types it themselves.
    try {
      const prefillEmail = (new URLSearchParams(window.location.search).get('email') || '').trim();
      if (prefillEmail && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(prefillEmail)) {
        emailInput.value = prefillEmail.toLowerCase();
      }
    } catch (_) { /* SSR-safe: window.location.search not present */ }

    // Focus the email field for instant typing (or the Continue button if
    // we already pre-filled the email so they don't tab out of the field).
    setTimeout(() => {
      if (emailInput.value) submitBtn.focus();
